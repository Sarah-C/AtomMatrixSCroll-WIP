...
void Scrolly::printChars(char *buffer, uint8_t length) {
  uint8_t i = 0;
  while (buffer[i] != 0) {
    this->addChar(buffer[i]);
    buffer++;
  }
}

void Scrolly::setFontColor(uint8_t r, uint8_t g, uint8_t b) {
  this->fontColor.r = r;
  this->fontColor.g = g;
  this->fontColor.b = b;
}
...
int Scrolly::findChar(char ascii) {
  ...
  return this->findChar(32); // Rturn index for the space.
}

void Scrolly::addChar(char c) {
  if (c > 96) c -= 32;
